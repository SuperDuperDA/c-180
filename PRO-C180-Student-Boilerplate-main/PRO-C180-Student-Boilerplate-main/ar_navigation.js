   ...
   $.ajax({
    ...
    success: function(response){
        let images = {
            ...
            "slight_;left": "ar_slight_left.png",
            "straight": "ar_straight.png"
        }
        let steps = response.routes[0].legs[0].steps
        for (let i = 0; i < steps.length; i++){
            let image;
            let distance = steps[i].maneuver.instruction
            ...
            if (i > 0) {
                $("#scene_container").append(
                    `<a-entity gps-entity-place="latitude: ${steps[i].maneuver.location[1]}; longitude: ${steps[i.manuver.location[0]]};">
                        <a-image
                            name="${instruction}"
